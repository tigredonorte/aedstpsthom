...
void setW(PFila cel1, double dist)
{
    ...
    if(cel1->item.distW > dist || cel1->item.distW <= 0)//< 0 se for a primeira inicializacao, 0 caso seja o mesmo ponto
    {
...
{
    //se a celula nao foi iniciada,ou se o o valor guardado e maior do que o valor de parametro
    if(cel1->item.distU > dist || cel1->item.distU <= 0)//< 0 se for a primeira inicializacao, 0 caso seja o mesmo ponto
    {
...
void calculaUW(Fila *fila, Fila *filaAmostra, Fila *filaGerada, int numDim)
{
    //elementos que percorrerao a fila
    PFila cel1, cel2;
    getFirstElement(fila, &cel1);
